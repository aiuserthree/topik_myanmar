/* ============================================================
   panels/permissions.js — 관리자 권한 관리 (vanilla port of permissions.jsx, TPKM_BO_6_5_*)
   편집형 권한 매트릭스:
...
function TriCheck({ checked, indeterminate, disabled, onChange, label }) {
  ...
  return h('label', { style: { display: 'inline-flex', alignItems: 'center', gap: 6, cursor: disabled ? 'default' : 'pointer', opacity: disabled ? 0.5 : 1 } },
    h('input', { ref: ref, type: 'checkbox', checked: checked, disabled: disabled, onChange: onChange, style: { width: 16, height: 16 } }),
    label && h('span', { style: { fontSize: 12.5, fontWeight: 600, color: 'var(--text-2)' } }, label)
  );
}
...
function PermissionsPanel() {
  ...
  return h(Fragment, null,
    h('div', { className: 'panel-head' },
      h('div', null,
        h('h1', null, '관리자 권한 관리'),
        h('div', { className: 'sub' }, '등급 선택 후 각 메뉴의 권한 액션을 체크/해제하세요 · 섹션 「전체선택」으로 구분 단위 일괄 적용 · 최고관리자만 편집')
      ),
      h('div', { className: 'actions' },
        h('button', { className: 'btn btn-secondary', onClick: exportMatrix }, h(I.Download, { style: { width: 14, height: 14 } }), ' 내보내기')
      )
    ),
    ...
    // 등급 선택 + 권장값
    h('div', { className: 'acard', style: { marginBottom: 16 } },
      h('div', { className: 'acard-body', style: { display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' } },
        h('div', { className: 'seg' },
          ROLE_DEFS.map(r => h('button', { key: r.role, className: role === r.role ? 'active' : '', onClick: () => setRole(r.role) },
            r.title, ' ', h('span', { style: { fontFamily: 'Inter', opacity: 0.6, marginLeft: 2 } }, roleCount[r.role])
          ))
        ),
        h('div', { style: { flex: 1, minWidth: 200, fontSize: 12.5, color: 'var(--text-3)' } },
          h('span', { className: `tag ${ROLE_DEFS.find(r => r.role === role).tag}`, style: { marginRight: 8 } }, DataStore.roleLabel(role)),
          '권장: ', ROLE_DEFS.find(r => r.role === role).rec, ' · 현재 ', h('b', { style: { color: 'var(--text-2)', fontFamily: 'Inter' } }, totalOn), '개 액션 허용'
        ),
        h('button', { className: 'ibtn', disabled: !canManage, onClick: applyRecommended }, h(I.RefreshCcw, { style: { width: 13, height: 13 } }), ' 권장값 적용')
      )
    ),
    ...
    h('div', { className: 'acard' },
      h('div', { className: 'acard-head' },
        h('h3', null, DataStore.roleLabel(role), ' 권한 설정'),
        h('span', { className: 'meta' }, '구분(섹션)별 「전체선택」 지원')
      ),
      h('div', { className: 'acard-body flush' },
        sections.map(sec => {
          const ss = sectionState(sec);
          return h('div', { key: sec.id, style: { borderBottom: '1px solid var(--border)' } },
            // section header
            h('div', { style: { display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', background: '#FAFBFD' } },
              h(TriCheck, { checked: ss.all, indeterminate: ss.some, disabled: !canManage,
                onChange: () => toggleSectionAll(sec, !ss.all),
                label: `${sec.title}  ·  전체선택` }),
              h('span', { style: { fontSize: 11.5, color: 'var(--text-3)' } }, sec.menus.length, '개 메뉴')
            ),
            // menu rows
            sec.menus.map(m => {
              const ms = menuState(m);
              const have = cur[m.id] || [];
              return h('div', { key: m.id, style: { display: 'grid', gridTemplateColumns: '200px 1fr', gap: 12, padding: '10px 16px', alignItems: 'center', borderTop: '1px solid var(--border)' }, className: 'perm-row' },
                h('div', { style: { display: 'flex', alignItems: 'center', gap: 8 } },
                  h(TriCheck, { checked: ms.all, indeterminate: ms.some, disabled: !canManage,
                    onChange: () => toggleMenuAll(m, !ms.all) }),
                  h('span', { style: { fontSize: 13.5, fontWeight: 600, color: 'var(--text)' } }, m.label)
                ),
                h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6 } },
                  m.actions.map(a => {
                    const on = have.includes(a);
                    return h('button', { key: a, type: 'button', disabled: !canManage,
                      onClick: () => toggleAction(m, a),
                      className: 'perm-chip',
                      'data-on': on },
                      h('span', { className: 'bx' }, on && h(I.Check, { style: { width: 11, height: 11 } })),
                      actLabel(a)
                    );
                  })
                )
    ...
    // sticky save bar
    canManage && h('div', { style: { position: 'sticky', bottom: 0, marginTop: 16, padding: '12px 16px', background: '#fff', border: '1px solid var(--border)', borderRadius: 10, display: 'flex', alignItems: 'center', gap: 12, boxShadow: 'var(--shadow)' } },
      h('div', { style: { fontSize: 13, color: dirty ? 'var(--accent)' : 'var(--text-3)' } },
        dirty ? '● 저장되지 않은 변경 사항이 있습니다.' : '변경 사항 없음 · 모든 변경은 처리 이력에 기록됩니다.'
      ),
      h('div', { style: { marginLeft: 'auto', display: 'flex', gap: 8 } },
        h('button', { className: 'btn btn-secondary', disabled: !dirty, onClick: resetDraft }, '되돌리기'),
        h('button', { className: 'btn btn-primary', disabled: !dirty, onClick: save }, '변경 사항 저장')
      )
    ),

    h('div', { style: { marginTop: 16, padding: 14, background: 'var(--bg-2)', borderRadius: 8, fontSize: 12.5, color: 'var(--text-3)', lineHeight: 1.7 } },
      '등급 변경(계정별)은 ', h('a', { href: '#admins', style: { color: 'var(--primary)' } }, '관리자 계정 관리'), '에서 수행합니다. 권한 검증은 서버에서 강제됩니다(클라이언트 메뉴 숨김만으로 보호되지 않음). 세밀한 RBAC는 향후 확장 항목입니다.'
    ),
