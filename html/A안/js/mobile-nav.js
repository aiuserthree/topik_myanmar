(function () {
  ...
  function wireDrawerLinks() {
    ...
    nav.querySelectorAll("a").forEach(function (a) {
      // 1depth 아코디언 토글 링크(.gnb-top)는 드로어를 닫지 않음 —
      // fo-nav.js wireMegaMenu()가 e.preventDefault() + drawer-open 토글만 수행.
      // 드로어는 열린 채 아코디언만 펼쳐져야 함.
      if (a.classList.contains("gnb-top")) return;
      // 중복 등록 방지
      if (a.dataset.tmCloseWired === "1") return;
      a.dataset.tmCloseWired = "1";
  ...
  document.addEventListener("tm-fo-nav-ready", function () {
    relocateLangSwitch();
    wireDrawerLinks();
    if (window.TMI18N) TMI18N.apply(TMI18N.getLang());
  });
