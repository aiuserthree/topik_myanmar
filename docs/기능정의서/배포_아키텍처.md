...
| 옵션 | FO 정적 | API | 비고 |
|------|---------|-----|------|
| A | Vercel | AWS/GCP VM + Nginx + FastAPI/Django/Node | 전통 3-tier |
| B | Vercel | Railway / Render / Fly.io | 소규모 팀에 단순 |
| C | Vercel | 동일 클라우드 PaaS (Elastic Beanstalk, Cloud Run) | 운영 정책에 따름 |

- **웹 서버(197):** FO는 Vercel CDN; API 앞단 Nginx(또는 매니지드 LB) 리버스 프록시
- **앱 서버(198):** 언어·프레임워크는 운영·개발 역량 합의 후 확정
